...
export class ReservationContentsRoutes {
	static async register(app: Express) {
		...
		app.post("/reservation_contents/addType", async (req, res) => {
			const query = 'INSERT INTO reservation_contents(reservationID, typeID, quantity) VALUES ("' + req.body.reservationID + '", "' + req.body.typeID + '", "' + req.body.quantity + '")'
		...
		app.get("/reservation_contents/reservation/all/:reservationID", async (req, res) => {
