...
export class ItemKitContentRoutes {
	static async register(app: Express) {
		...
		app.get("/itemKitContent/kit/type/:kitID", async (req, res) => {
			...
			const query = 'SELECT ikc.typeID,ikc.qty,it.name FROM itemKitContent ikc LEFT JOIN item_type it USING(typeID) WHERE ikc.kitID = ' + ID;
		...
		app.post("/itemKitContent/addItem", async (req, res) => {
			const query = 'INSERT INTO itemKitContent (typeID, kitID, qty) VALUES ("' + req.body.typeID + '", "' + req.body.kitID + '", "' + req.body.qty + '")'
			mysql.query(query, (err:Error, results:object[])=>{
				if(err){
					res.status(400).json({
						ok: false,
